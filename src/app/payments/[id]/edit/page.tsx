...
export default function EditPaymentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [customersRes, jobsRes, paymentRes] = await Promise.all([
        ...
        setValue('customer_id', payment.customer_id)
        setValue('job_id', payment.job_id ?? 'none')
        ...
        toast.error('Ödeme bilgileri yüklenemedi', {
          description: message,
        })
  ...
  if (isLoadingData) {
    return (
      <MainLayout title="Ödeme Düzenle" description="Yükleniyor...">
  ...
  return (
    <MainLayout title="Ödeme Düzenle" description="Ödeme bilgilerini güncelle">
